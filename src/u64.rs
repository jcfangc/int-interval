...
mod interval_arithmetic {
    ...
    impl U64CO {
        #[inline]
        pub const fn scale(self, factor: u64) -> Option<Self> {
            if factor == 0 || self.end_excl > u64::MAX / factor {
                return None;
            }
            Some(U64CO::new_unchecked(
                self.start * factor,
        ...
        #[inline]
        pub const fn shift(self, offset: u64) -> Option<Self> {
            if self.end_excl > u64::MAX - offset {
                return None;
            }
            Some(U64CO::new_unchecked(
                self.start + offset,
